...
function drawPipe(OD, ID, t) {
  ...
  ctx.setLineDash([4, 3]); // tratteggio sottile
  ctx.strokeStyle = "rgba(148, 163, 184, 0.5)"; // grigio-azzurro tenue
  ctx.lineWidth = 0.8;

  // Extension lines OD – verticali dall'asse orizzontale del tubo (cy) fino alla linea di quota OD in alto
  ctx.beginPath();
  ctx.moveTo(cx - Rext, cy);               // dall'asse orizzontale (sinistra)
  ctx.lineTo(cx - Rext, cy - Rext - 16);   // su fino alla linea di quota
  ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(cx + Rext, cy);               // dall'asse orizzontale (destra)
  ctx.lineTo(cx + Rext, cy - Rext - 16);
  ctx.stroke();

  // Extension lines ID – verticali dall'asse orizzontale del tubo (cy) fino alla linea di quota ID in basso
  ctx.beginPath();
  ctx.moveTo(cx - Rint, cy);               // dall'asse orizzontale (sinistra)
  ctx.lineTo(cx - Rint, cy + Rext + 20);   // giù fino alla linea di quota
  ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(cx + Rint, cy);               // dall'asse orizzontale (destra)
  ctx.lineTo(cx + Rint, cy + Rext + 20);
  ctx.stroke();

  // Extension lines spessore – piccoli tratti verticali sul lato destro
  ctx.beginPath();
  ...
  ctx.setLineDash([8, 4, 2, 4]); // tratto-punto (centro linea CAD)
  ctx.lineWidth = 0.6;
  // asse orizzontale
  ctx.beginPath();
  ...
  ctx.setLineDash([]); // reset a linea continua
  // ===== FINE LINEE CAD =====

  ctx.font = "bold 13px system-ui";
  ctx.fillStyle = "#e2e8f0"; // Testo bianco-grigio chiaro
  ...
  // spessore
  drawDoubleArrow(cx + Rint + 4, cy, cx + Rext - 4, cy, "#f43f5e", 2); // Rosso rosato neon
...
function drawDoubleArrow(x1, y1, x2, y2, color = "#e2e8f0", width = 1) {
  ...
  // Aggiungi un piccolo glow speculativo anche alle frecce
