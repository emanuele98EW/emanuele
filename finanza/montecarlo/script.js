...
function runSimulation() {
  const capIniziale = parseItaNumber(document.getElementById('capitale').value) || 0;
  const versAnnuo = parseItaNumber(document.getElementById('versamento').value) || 0;
  const mu = (parseItaNumber(document.getElementById('rendimento').value) || 0) / 100;
  const sigma = (parseItaNumber(document.getElementById('volatilita').value) || 0) / 100;
  const anni = Math.round(parseItaNumber(document.getElementById('anni').value)) || 10;
  let N = Math.round(parseItaNumber(document.getElementById('simulazioni').value)) || 1000;
...
function calcola() {
  ...
  cardData.forEach(c => {
    ...
    let glow = c.highlight ? `text-shadow:0 0 15px rgba(59,130,246,0.5);` : '';
    let pColor = c.highlight ? 'var(--text-primary)' : c.color; // Use c.color for non-highlighted, var(--text-primary) for highlighted
    cardsContainer.innerHTML += `
      <div class="card finance" style="padding:20px; text-align:center; ${styleExtra}">
        ...
        <p style="font-size:1.6rem; color:${pColor}; font-family:'Orbitron',sans-serif; ${glow}">${fmt(c.val)}</p>
      </div>`;
  });
